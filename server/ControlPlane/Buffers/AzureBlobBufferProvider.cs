...
public sealed class AzureBlobBufferProvider : IHostedService, IBufferProvider, IHealthCheck
{
    ...
    public async Task<Buffer> CreateBuffer(Buffer buffer, CancellationToken cancellationToken)
    {
        ...
        buffer = buffer with { StorageAccountId = storageAccountId, Location = location };
        return await _repository.CreateBuffer(buffer, cancellationToken);
    }
    ...
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        Dictionary<int, string> idToNameMap = await _repository.UpsertStorageAccounts(_storageOptions.StorageAccounts.Select(sa => (sa.Name, sa.Endpoint, sa.Location)).ToList(), cancellationToken);
    ...
    public IList<StorageAccountInfo> GetStorageAccounts()
    {
        return _storageOptions.StorageAccounts.Select(sa => new StorageAccountInfo(sa.Name, sa.Location, sa.Endpoint)).ToList();
    }
