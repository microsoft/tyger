...
public class Repository
{
                ...
                Parameters =
                    {
                        ...
                        new() { Value = newBuffer.StorageAccountId, NpgsqlDbType = NpgsqlDbType.Integer },
                    }
            };
    ...
    public async Task<Dictionary<int, string>> UpsertStorageAccounts(IList<(string name, string endpoint, string location)> storageAccounts, CancellationToken cancellationToken)
    {
        return await _resiliencePipeline.ExecuteAsync(async cancellationToken =>
        {
            await using var connection = await _dataSource.OpenConnectionAsync(cancellationToken);
