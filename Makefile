...
set-env:
	...
	postgres_password=$(shell kubectl get secrets ${HELM_RELEASE}-tygerdb -o jsonpath="{.data.postgresql-password}" | base64 -d)
	cat <<- EOF > .env
		...
		TYGER_DATABASE_CONNECTION_STRING="host=tyger-tygerdb dbname=tyger port=5432 user=postgres password=$${postgres_password}"
...
docker-build:
	...
	docker build -t tyger --target tyger .
	docker build -t testrecon:test --target testrecon .

up: docker-build
	tyger_image_id=$$(docker inspect tyger | jq -r '.[0].Id')
	...
	helm upgrade --install \
				...
				"${HELM_RELEASE}" \
				"${HELM_CHART_DIR}" \
				--set tyger.image="$${tyger_image_id}" \
				--set tyger.hostname="${TYGER_HOSTNAME}" \
