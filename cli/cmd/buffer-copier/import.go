...
func newImportCommand(dbFlags *databaseFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			err = bulkInsert(ctx, pool, cred, dbBatchSize, storageAccountId, containerChannel)
...
func bulkInsert(ctx context.Context, pool *pgxpool.Pool, cred azcore.TokenCredential, batchSize int, storageAccountId int, containers <-chan *service.ContainerItem) error {
	totalCount := int64(0)
