...
func newExportCommand(dbFlags *databaseFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			for page, err := range getBufferIdsAndTags(ctx, pool, sourceStorageAccountId, filter, cred) {
				if err != nil {
...
func getBufferIdsAndTags(ctx context.Context, pool *pgxpool.Pool, sourceStorageAccountId int, filter map[string]string, cred azcore.TokenCredential) iter.Seq2[[]bufferIdAndTags, error] {
