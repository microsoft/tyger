...
func TestConvertConfig(t *testing.T) {
	...
	authSpecPath := path.Join(tempdir, "authspec.yml")

	authSpecFile, err := os.OpenFile(authSpecPath, os.O_RDWR|os.O_CREATE|os.O_TRUNC, 0644)
	require.NoError(t, err)
	defer authSpecFile.Close()

	cloudinstall.PrettyPrintAuthSpec(authSpec, authSpecFile)
	authSpecFile.Close()

	errorBuf := bytes.Buffer{}
	ctx := zerolog.New(&errorBuf).WithContext(context.Background())
	require.NoError(t, convert(ctx, oldPath, newPath, authSpecPath), errorBuf.String())
}
