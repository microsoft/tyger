...
func NewBufferCommand() *cobra.Command {
	...
	cmd.AddCommand(newListStorageAccountsCommand())
...
func newListStorageAccountsCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:                   "list-storage-accounts",
		Short:                 "List storage accounts",
		Long:                  `List storage accounts.`,
		DisableFlagsInUseLine: true,
		RunE: func(cmd *cobra.Command, args []string) error {
			storageAccounts := []model.StorageAccountInfo{}
