	...
	containerSpec := containerSpec{
		ContainerConfig: &container.Config{
			...
			Env: []string{
				...
				"Buffers__ActiveLifetime=" + inst.Config.Buffers.ActiveLifetime,
			...
			Healthcheck: &container.HealthConfig{
				Test: []string{
					...
					"/app/bin/curl",
					"--fail",
					"--unix-socket", fmt.Sprintf("%s/control-plane/tyger.sock", inst.Config.InstallationPath),
		...
		ContainerConfig: &container.Config{
			...
			Healthcheck: &container.HealthConfig{
				Test: []string{
					...
					"/app/bin/curl",
					"--fail",
					"--unix-socket", fmt.Sprintf("%s/data-plane/tyger.data.sock", inst.Config.InstallationPath),
					"http://local/healthcheck",
				},
				StartInterval: 2 * time.Second,
				Interval:      10 * time.Second,
