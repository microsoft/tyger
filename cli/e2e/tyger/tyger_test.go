...
import (
	...
	"dev.azure.com/msresearch/compimag/_git/tyger/cli/e2e"
...
func init() {
	stdout, stderr, err := e2e.RunTyger("login", "status")
	if err != nil {
...
func TestEndToEnd(t *testing.T) {
	...
	inputBufferId := e2e.RunTygerSuceeds(t, "buffer", "create")
	inputSasUri := e2e.RunTygerSuceeds(t, "buffer", "access", inputBufferId, "-w")

	// create and output buffer and a SAS token to be able to read from it
	outputBufferId := e2e.RunTygerSuceeds(t, "buffer", "create")
	outputSasUri := e2e.RunTygerSuceeds(t, "buffer", "access", outputBufferId)

	e2e.RunCommandSuceeds(t, "sh", "-c", fmt.Sprintf(`echo "Hello" | buffer-proxy write "%s"`, inputSasUri))

	// create run
	runId := e2e.RunTygerSuceeds(t, "run", "create", "--codespec", codespecName, "--timeout", "10m",
		"-b", fmt.Sprintf("input=%s", inputBufferId),
		"-b", fmt.Sprintf("output=%s", outputBufferId))

	waitForRunSuccess(t, runId)

	output := e2e.RunCommandSuceeds(t, "sh", "-c", fmt.Sprintf(`buffer-proxy read "%s"`, outputSasUri))
...
func TestInvalidCodespecNames(t *testing.T) {
	...
	for _, tC := range testCases {
		t.Run(tC.name, func(t *testing.T) {
			_, stdErr, err := e2e.RunTyger("codespec", "create", tC.name, "--image", "busybox")
			if tC.valid {
				assert.Nil(t, err)
			} else {
				assert.NotNil(t, err)
			...
			_, err = tyger.InvokeRequest(http.MethodPut, fmt.Sprintf("v1/codespecs/%s", tC.name), newCodespec, nil, false)
			if tC.valid {
				assert.Nil(t, err)
			} else {
				assert.NotNil(t, err)
...
func TestCodespecNameRequirements(t *testing.T) {
	e2e.RunTyger("codespec", "create", "Foo", "--image", "busybox")
}
...
func TestTargetGpuNodePool(t *testing.T) {
	...
	// create run
	runId := e2e.RunTygerSuceeds(t, "run", "create", "--codespec", codespecName, "--node-pool", "gpunp", "--timeout", "20m")
...
func TestTargetCpuNodePool(t *testing.T) {
	...
	// create run
	runId := e2e.RunTygerSuceeds(t, "run", "create", "--codespec", codespecName, "--node-pool", "cpunp", "--timeout", "10m")
...
func TestTargetingInvalidClusterReturnsError(t *testing.T) {
	...
	_, stderr, _ := e2e.RunTyger("run", "create", "--codespec", codespecName, "--cluster", "invalid")
...
func TestTargetingInvalidNodePoolReturnsError(t *testing.T) {
	...
	_, stderr, _ := e2e.RunTyger("run", "create", "--codespec", codespecName, "--node-pool", "invalid")
...
func TestTargetCpuNodePoolWithGpuResourcesReturnsError(t *testing.T) {
	...
	_, stderr, _ := e2e.RunTyger("run", "create", "--codespec", codespecName, "--node-pool", "cpunp", "--timeout", "10m")
...
func TestUnrecognizedFieldsRejected(t *testing.T) {
	...
	_, err := tyger.InvokeRequest(http.MethodPut, "v1/codespecs/tcs", requestBody, &codespec, false)
	require.Nil(err)
...
func TestResponseContainsRequestIdHeader(t *testing.T) {
	t.Parallel()
	require := require.New(t)
	_, stderr, _ := e2e.RunTyger("codespec", "show", "missing")
...
func TestListRunsPaging(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		runs[e2e.RunTygerSuceeds(t, "run", "create", "--codespec", "exitimmediately", "--timeout", "10m")] = ""
	}

	for uri := "v1/runs?limit=5"; uri != ""; {
		page := model.Page[model.Run]{}
		_, err := tyger.InvokeRequest(http.MethodGet, uri, nil, &page, false)
		require.Nil(t, err)
...
func TestListCodespecsFromCli(t *testing.T) {
	...
	for _, name := range codespecNames {
		codespecMap[name] = e2e.RunTygerSuceeds(t, "codespec", "create", name, "--image", "busybox")
	}
	var results = e2e.RunTygerSuceeds(t, "codespec", "list", "--prefix", prefix)
...
func TestRecreateCodespec(t *testing.T) {
	t.Parallel()
	codespecName := strings.ToLower(t.Name())
	version1 := e2e.RunTygerSuceeds(t, "codespec", "create", codespecName, "--image", "busybee", "--command", "--", "echo", "hi I am first")
	version2 := e2e.RunTygerSuceeds(t, "codespec", "create", codespecName, "--image", "busybox", "--gpu", "2", "--memory-request", "2048048", "--env", "os=ubuntu", "--command", "--", "echo", "hi I am latest")
	require.NotEqual(t, version1, version2)

	version3 := e2e.RunTygerSuceeds(t, "codespec", "create", codespecName, "--image", "busybox", "--gpu", "2", "--memory-request", "2048048", "--env", "os=ubuntu", "--command", "--", "echo", "hi I am latest")
	require.Equal(t, version2, version3)

	version4 := e2e.RunTygerSuceeds(t, "codespec", "create", codespecName, "--image", "busybox", "--memory-request", "2048048", "--gpu", "2", "--env", "os=ubuntu", "--command", "--", "echo", "hi I am latest")
	require.Equal(t, version3, version4)

	version5 := e2e.RunTygerSuceeds(t, "codespec", "create", codespecName, "--image", "busybox", "--memory-request", "2048048", "--gpu", "2", "--env", "os=ubuntu", "--env", "platform=highT", "--command", "--", "echo", "hi I am latest")
	version6 := e2e.RunTygerSuceeds(t, "codespec", "create", codespecName, "--image", "busybox", "--gpu", "2", "--memory-request", "2048048", "--env", "platform=highT", "--env", "os=ubuntu", "--command", "--", "echo", "hi I am latest")
	require.Equal(t, version5, version6)

	version7 := e2e.RunTygerSuceeds(t, "codespec", "create", codespecName, "--image", "busybox", "--memory-request", "2048048", "--gpu", "2", "--env", "platform=highT", "--env", "os=windows", "--command", "--", "echo", "hi I am latest")
...
func TestListCodespecsPaging(t *testing.T) {
	...
	for _, name := range inputNames {
		codespecs[name] = e2e.RunTygerSuceeds(t, "codespec", "create", name, "--image", "busybox")
	}
	...
	for uri := fmt.Sprintf("v1/codespecs?limit=5&prefix=%s", prefix); uri != ""; {
		...
		for _, cs := range page.Items {
			...
			if expectedIdx == 6 && expectedKlamathVersion == 0 {
				var tmp = e2e.RunTygerSuceeds(t, "codespec", "create", prefix+"klamath", "--image", "busybox", "--", "something different")
...
func TestListRunsSince(t *testing.T) {
	...
	e2e.RunTygerSuceeds(t, "run", "create", "--codespec", codespecName, "--timeout", "10m")
	midId := e2e.RunTygerSuceeds(t, "run", "create", "--codespec", codespecName, "--timeout", "10m")
	lastId := e2e.RunTygerSuceeds(t, "run", "create", "--codespec", codespecName, "--timeout", "10m")

	midRunJson := e2e.RunTygerSuceeds(t, "run", "show", midId)
	midRun := model.Run{}
	err := json.Unmarshal([]byte(midRunJson), &midRun)
	require.Nil(t, err)

	listJson := e2e.RunTygerSuceeds(t, "run", "list", "--since", midRun.CreatedAt.Format(time.RFC3339Nano))
...
func TestListCodespecsWithPrefix(t *testing.T) {
	...
	codespecMap := make(map[string]string)
	for i := 0; i < 4; i++ {
		codespecMap[codespecNames[i]] = e2e.RunTygerSuceeds(t, "codespec", "create", codespecNames[i], "--image", "busybox")
	}
...
func TestGetLogsFromPod(t *testing.T) {
	...
	resp, err := tyger.InvokeRequest(http.MethodGet, fmt.Sprintf("v1/runs/%s/logs?follow=true", runId), nil, nil, false)
	require.Nil(t, err)
	...
	logs := e2e.RunTygerSuceeds(t, "run", "logs", runId)
	require.Equal(t, "1\n2\n3\n4\n5", logs)

	// --timestamp should prefix each line with its timestamp
	logs = e2e.RunTygerSuceeds(t, "run", "logs", runId, "--timestamps")
	...
	logs = e2e.RunTygerSuceeds(t, "run", "logs", runId, "--since", firstTimestamp.Add(time.Second).Format(time.RFC3339Nano))
	require.NotContains(t, logs, "1")

	logs = e2e.RunTygerSuceeds(t, "run", "logs", runId, "--tail", "3")
	require.Equal(t, "3\n4\n5", logs)
}
...
func TestGetArchivedLogs(t *testing.T) {
	...
	t.Logf("Run ID: %s", runId)
	waitForRunStarted(t, runId)
	logs := e2e.RunTygerSuceeds(t, "run", "logs", runId, "--follow")
	require.Equal(t, "1\n2\n3\n4", logs)

	waitForRunSuccess(t, runId)

	// force logs to be archived
	_, err := tyger.InvokeRequest(http.MethodPost, "v1/runs/_sweep", nil, nil, false)
	require.Nil(t, err)
	...
	// --since
	logs = e2e.RunTygerSuceeds(t, "run", "logs", runId, "--since", firstTimestamp.Format(time.RFC3339Nano))
	require.Equal(t, "2\n3\n4", logs)
	logs = e2e.RunTygerSuceeds(t, "run", "logs", runId, "--since", firstTimestamp.Add(time.Minute).Format(time.RFC3339Nano))
	...
	logs = e2e.RunTygerSuceeds(t, "run", "logs", runId, "--tail", "3")
	require.Equal(t, "2\n3\n4", logs)
	logs = e2e.RunTygerSuceeds(t, "run", "logs", runId, "--tail", "0")
	require.Equal(t, "", logs)
	logs = e2e.RunTygerSuceeds(t, "run", "logs", runId, "--tail", "4")
	require.Equal(t, "1\n2\n3\n4", logs)
}
...
func TestConnectivityBetweenJobAndWorkers(t *testing.T) {
	...
	digest := e2e.RunCommandSuceeds(t, "docker", "inspect", "testconnectivity", "--format", "{{ index .RepoDigests 0 }}")
	...
	runId := e2e.RunTygerSuceeds(t, "run", "create", "--codespec", jobCodespecName, "--worker-codespec", workerCodespecName, "--worker-replicas", "3", "--timeout", "10m")
	waitForRunSuccess(t, runId)
}
...
func waitForRunStarted(t *testing.T, runId string) {
	start := time.Now()
	for {
		runJson := e2e.RunTygerSuceeds(t, "run", "show", runId)
		run := model.Run{}
...
func waitForRunSuccess(t *testing.T, runId string) model.Run {
	...
	e2e.RunTygerSuceeds(t, "run", "logs", runId, "-f")
