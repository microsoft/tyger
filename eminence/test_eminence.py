...
def verify_basic_recon_results(testdata_filename, recon_filename):
    ...
    reconstruction = np.squeeze(recon_data['img']['image_0']['data'])
...
@pytest.mark.parametrize(
    "image",
    [
        get_dependency_image("recon"),
        get_dependency_image("python_recon"),
    ])
def test_simple_reconstruction(data_dir: Path, temp_output_filename: str, image: str):
    ...
    verify_basic_recon_results(test_file, temp_output_filename)
...
def test_noise_dependency_reconstruction(data_dir: Path, temp_output_filename: str, image: str, scanner_session_id: str):
    ...
    img_data = np.squeeze(recon_data['img']['image_0']['data'])
